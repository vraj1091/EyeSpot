  ...
  return (
    <motion.div initial="initial" animate="in" exit="out" variants={pageVariants} className="space-y-40 pb-20 pt-20">
      ...
      <section className="text-center max-w-5xl mx-auto space-y-10 relative z-10">
        <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="text-7xl md:text-9xl font-black bg-clip-text text-transparent bg-gradient-to-l from-white via-primary to-slate-900 drop-shadow-xl">
          Initialize Uplink
        </motion.h1>
      ...
      <section className="grid lg:grid-cols-2 gap-20 max-w-[1200px] mx-auto items-stretch">
        
        <motion.div initial={{ opacity: 0, x: -50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: '-50px' }} className="space-y-16">
          <div className="space-y-6">
            <h3 className="text-5xl font-black tracking-tight text-white mb-2">Direct Vectors</h3>
            <p className="text-gray-400 text-2xl font-light leading-relaxed">Establish a secure comms channel to schedule pilot mapping.</p>
          </div>
        ...
        <motion.div initial={{ opacity: 0, x: 50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: '-50px' }}>
          <div className="glass-panel p-16 rounded-[4rem] border-white/10 space-y-12 relative overflow-hidden h-full shadow-[0_30px_60px_rgba(0,0,0,0.5)]">
            <div className="absolute -top-40 -right-40 w-[500px] h-[500px] bg-primary/10 rounded-full blur-[100px] pointer-events-none" />
            <h3 className="text-4xl font-black tracking-tight text-white mb-4 relative z-10">Neural Transmission</h3>
            ...
            <form className="space-y-8 relative z-10" onSubmit={e => e.preventDefault()}>
              ...
              <button className="w-full py-6 rounded-2xl font-black tracking-widest uppercase text-xl bg-white text-slate-950 hover:bg-primary shadow-[0_15px_30px_rgba(255,255,255,0.2)] hover:shadow-[0_20px_50px_rgba(56,189,248,0.5)] flex items-center justify-center gap-4 transition-all duration-500 hover:scale-[1.02]">
                <Send className="w-6 h-6" /> Beam Data
              </button>
            </form>
          </div>
        </motion.div>
