  ...
  return (
    <motion.div initial="initial" animate="in" exit="out" variants={pageVariants} className="space-y-40 pb-20 pt-20 relative">
      ...
      <section className="text-center max-w-5xl mx-auto space-y-6 md:space-y-10 relative z-10 px-4">
        <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="text-5xl sm:text-7xl lg:text-9xl font-black bg-clip-text text-transparent bg-gradient-to-br from-white via-primary to-slate-900 drop-shadow-xl leading-tight pb-2">
      ...
      <section className="grid lg:grid-cols-2 gap-8 md:gap-16 max-w-7xl mx-auto items-stretch px-4">
        {products.map((p, i) => {
          ...
          return (
            <motion.article 
              ...
              className="glass-panel p-8 md:p-14 rounded-[2.5rem] md:rounded-[3rem] flex flex-col hover:-translate-y-2 transition-all duration-500 border-white/5 hover:border-primary/40 group bg-slate-900/40 relative overflow-hidden"
            >
              ...
              <header className="flex flex-col sm:flex-row items-start sm:items-center gap-6 mb-8 relative z-10">
                <div className="bg-slate-900 border border-white/10 w-20 h-20 md:w-24 md:h-24 rounded-[1.5rem] md:rounded-3xl flex items-center justify-center text-primary group-hover:scale-110 group-hover:bg-primary group-hover:text-slate-950 transition-all duration-700 shadow-2xl rotate-3 group-hover:rotate-0 flex-shrink-0">
                  <IconComponent className="w-8 h-8 md:w-10 md:h-10 drop-shadow-lg" />
                </div>
                <h2 className="text-2xl md:text-4xl font-black text-white tracking-tight group-hover:text-primary transition-colors leading-tight">
                  {p.name}
                </h2>
              </header>
              
              <div className="relative z-10 space-y-6 md:space-y-8 flex-1">
                <p className="text-gray-300 text-xl md:text-2xl font-light italic border-l-4 border-primary/40 pl-4 md:pl-6 leading-relaxed">
                  {p.description}
                </p>
                
                <p className="text-gray-400 text-base md:text-lg leading-loose font-medium">
