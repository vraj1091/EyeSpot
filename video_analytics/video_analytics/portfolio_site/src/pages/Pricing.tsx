  ...
  return (
    <motion.div initial="initial" animate="in" exit="out" variants={pageVariants} className="space-y-40 pb-20 pt-20">
      ...
      <section className="text-center max-w-5xl mx-auto space-y-10 relative z-10 px-4">
        <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="text-6xl md:text-8xl font-black bg-clip-text text-transparent bg-gradient-to-tr from-white via-primary to-slate-800 drop-shadow-xl">
          Scale The Protocol
        </motion.h1>
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="text-xl md:text-2xl text-gray-400 font-light leading-relaxed max-w-3xl mx-auto">
        ...
        {/* Cycle Toggle */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="inline-flex items-center gap-1 bg-slate-900 border border-white/10 p-2 rounded-full shadow-[0_10px_30px_rgba(0,0,0,0.5)]">
      ...
      <section className="max-w-[1600px] mx-auto px-4">
        <AnimatePresence mode="wait">
          <motion.div 
            ...
            className="grid md:grid-cols-2 xl:grid-cols-4 gap-8 items-stretch"
          >
            {filteredPlans.map((plan, i) => {
              ...
              return (
                <div 
                  key={plan.id}
                  className={`p-10 rounded-[2.5rem] border relative flex flex-col transition-all duration-700 ${
                    recommended 
                  ...
                  <div className={`relative z-10 flex-col flex h-full ${recommended ? 'pt-6' : ''}`}>
                    <div className="mb-8">
                      <div className="flex justify-between items-center mb-4">
                        ...
                        <Hexagon className={`w-8 h-8 ${recommended ? 'text-primary' : 'text-gray-600'}`} />
                      </div>
                      <div className="text-4xl md:text-5xl lg:text-5xl font-black tracking-tighter text-transparent bg-clip-text bg-gradient-to-br from-white to-gray-500 drop-shadow-xl align-baseline leading-none pt-2 pb-2">
                        {plan.price}
                      </div>
        ...
        <div className="mt-20 glass-panel p-12 rounded-[3rem] border-accent/20 flex flex-col md:flex-row items-center justify-between gap-10 bg-slate-900/60 shadow-[0_0_80px_rgba(129,140,248,0.1)]">
          <div className="space-y-4 max-w-2xl">
            <h3 className="text-4xl font-black tracking-tighter bg-clip-text text-transparent bg-gradient-to-r from-accent to-primary">Custom AI Training Ops</h3>
            <p className="text-gray-400 text-lg leading-relaxed">Require specialized detection mechanics? We ingest your proprietary video data and train elite, highly specific logic models exclusively tailored for your physical infrastructure and compliance needs.</p>
          </div>
          <button className="px-10 py-5 bg-white text-slate-950 font-black tracking-widest uppercase rounded-full hover:bg-accent hover:text-white transition-all shadow-[0_15px_30px_rgba(255,255,255,0.2)] hover:shadow-[0_20px_40px_rgba(129,140,248,0.6)]">
